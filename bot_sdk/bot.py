    ...
    async def _handle_perm(
        ...
        if action == "set_owner":
            if not arg1:
                await self.send_reply(
                    message, err(self.tr("Usage: !perm set_owner <user_id>"))
                )
                return
            try:
                new_owner = int(arg1)
        ...
        if action == "roles_set":
            if not arg1 or not arg2:
                await self.send_reply(
                    message, err(self.tr("Usage: !perm roles_set <role> <level>"))
                )
                return
            role = str(arg1)
            try:
        ...
        if action == "allow_stop":
            if not arg1:
                await self.send_reply(
                    message, err(self.tr("Usage: !perm allow_stop <user_id>"))
                )
                return
            try:
                uid = int(arg1)
        ...
        if action == "deny_stop":
            if not arg1:
                await self.send_reply(
                    message, err(self.tr("Usage: !perm deny_stop <user_id>"))
                )
                return
            try:
                uid = int(arg1)
